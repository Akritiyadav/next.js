    ...
    <ol className='list-decimal font-semibold'>
     {Object.keys(cart).map((k)=>{return  <li key={k}>
      ...
      <div className='  w-2/3 font-semibold'>{cart[k].name} </div>
        ...
        <AiFillMinusCircle className='cursor-pointer text-pink-500' /><span className='mx-2 text-sm'>{cart[k].qty}</span> 
        <AiFillPlusCircle className='cursor-pointer text-pink-500' /></div>
      </div>
